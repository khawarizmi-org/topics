...
- **أقصر مسار في الرسوم البيانية غير الموزونة (Shortest Path in Unweighted Graphs)**  
  بما أن تكلفة كل حافة (edge) متساوية، فإن التوسّع على شكل “حلقة” في BFS يضمن أنّ أول مرة يتم فيها الوصول إلى أي عقدة تتم عبر مسار بأقل عدد من الحواف. على سبيل المثال، إيجاد أسرع طريق عبر متاهة (grid) حيث كل حركة (حركة) في أي اتجاه تكلف نفس القيمة هو تطبيق مباشر لـ BFS.

- **المركبات المرتبطة (Connected Components)**  
  في رسم بياني غير موجه، يمكنك إيجاد جميع المركبات المرتبطة في زمن $O(n + m)$ عبر تشغيل BFS من كل رأس (vertex) لم يُزر بعد دون إعادة تهيئة مصفوفة `[]vis` بين الجولات. هذه “سلسلة من عمليات BFS” تضع علامة (vis) على كل رأس مرة واحدة تمامًا.

- **تعبئة الفيض (Flood Fill)**  
  عندما تحتاج إلى تلوين أو معالجة كل الخلايا (cells) في منطقة معينة (مثل تلوين جزء في صورة)، ينتشر BFS من الخلية الأساسية إلى جميع الخلايا المتصلة من نفس النوع.

- **أقل عدد من الحركات في فضاءات الحالة (Minimum Moves in State Spaces)**  
  نمذجة كل حالة في اللعبة (مثل تكوين ألغاز الانزلاق أو موقع الفارس على رقعة الشطرنج) كرأس (vertex)، والأحوال الصالحة للحركة كحواف. عندها يجد BFS أقل عدد من الحركات للوصول إلى الحالة المستهدفة.

- **0-1 BFS (الرسوم البيانية الموزونة بوزن 0 أو 1)**  
  إذا كانت الحواف لها أوزان 0 أو 1، يمكن تعديل BFS باستخدام قائمة مزدوجة النهاية (deque) لإيجاد أقصر المسارات في زمن $O(n + m)$ بدفع الحواف ذات الوزن 0 إلى الأمام وتلك ذات الوزن 1 إلى الخلف.

- **أقصر دورة في الرسوم البيانية الموجهة غير الموزونة (Shortest Cycle in Directed Unweighted Graphs)**  
  ابدأ BFS من كل رأس، وفي اللحظة التي تعود فيها لزيارة المصدر عبر مسار مختلف، تكون قد وجدت أقصر دورة تمر عبر هذا المصدر. كرّر العملية لكل مصدر واختر الدورية الأقصر.
