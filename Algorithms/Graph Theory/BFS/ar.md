# البحث بعرض أولاً (BFS)

## المقدمة

يُعتبر **البحث بعرض أولاً (Breadth-First Search - BFS)** من أكثر **خوارزميات استكشاف** الرسم البياني (graph) أساسيةً. يستكشف BFS الرسم البياني في “دوائر” (rings) ذات مسافات متزايدة انطلاقًا من العقدة المصدر (node) ويضمن أنه عند زيارة كل عقدة لأول مرة، يكون قد وُجد أقصر مسار (path) من المصدر لها من حيث عدد الحواف (edges). يعمل BFS بزمن $O(|V| + |E|)$ ويستخدم مساحة إضافية $O(|V|)$. يُستخدم هذا الأسلوب في العديد من تطبيقات الشبكات، وحل المتاهات، وتحليل الشبكات الاجتماعية، والمزيد.

---

## وصف الخوارزمية

تأخذ الخوارزمية مدخلات على شكل **رسم بياني غير موزون** (unweighted graph) ومعرف العقدة المصدر $s$. يمكن أن يكون الرسم البياني موجهًا (directed) أو غير موجه (undirected)، إذ يتعامل BFS معهما بنفس الطريقة.

يمكن تشبيه BFS بانتشار **الحريق** في الرسم البياني: في الخطوة الصفرية تكون العقدة المصدر $s$ “مشتعلة”. في كل خطوة لاحقة، تُشعل كل عقدة محترقة جيرانها غير المشتعلين. في كل تكرار، يتسع “حلقة النار” بمقدار حافة واحدة.
...
5. عند إفراغ $q$، تكون كل العقد التي يمكن الوصول إليها قد زُيّنت بترتيب أقصر مسار. مصفوفة `dis[]` تحوي المسافات، ومصفوفة `par[]` تساعد في إعادة بناء المسارات.

لرسوم بيانية **موزونة** (weighted graphs)، يلزم استخدام خوارزميات أقصر مسار مثل Dijkstra أو Bellman–Ford أو Floyd–Warshall؛ إذ إن ضمان أقصر مسار عبر BFS ينطبق فقط على الرسوم البيانية غير الموزونة.
...
- **أقصر مسار في رسم بياني غير موزون**  
- **المركبات المتصلة** (Connected Components) في رسم بياني غير موجه  
- فلاش **التعبئة** (Flood Fill) على المصفوفات  
- **أقل عدد حركات** (حركات) للوصول إلى حالة في الألغاز  
- **0-1 BFS** للأوزان 0/1  
- **أقصر دورة** في رسم بياني موجه غير موزون
